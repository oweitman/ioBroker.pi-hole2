...
module.exports = class PiholeClient {
     ...
     * @param {object} options.adapter - ioBroker adapter object used for managed timers.
     */
    constructor(options) {
    ...
    async makeRequest(method, path, body = null, query = null) {
        ...
        const timeout = this.adapter.setTimeout(() => controller.abort(), this.options.timeout ?? 10000);
    ...
    async setupSession() {
        ...
        const authCheckNeedsLogin = !authCheck.ok && authCheck.response?.statusCode === 401;

        if (!authCheck.ok && !authCheckNeedsLogin) {
            this.session = undefined;
            this.log.warn(
        ...
        if (authCheck.ok && authCheck.body?.session?.valid) {
    ...
    /**
     * Gets filter suggestions for the Pi-hole query log.
     *
     * @returns {Promise<object>} API result containing known client names and IP addresses.
     */
    async getQuerySuggestions() {
        return this.callAuthenticated('GET', '/queries/suggestions');
    }

    /**
     * Gets one page of query-log entries.
     *
     * @param {object} query - Supported /queries filter and pagination parameters.
     * @returns {Promise<object>} API result containing query-log entries.
     */
    async getQueries(query) {
        return this.callAuthenticated('GET', '/queries', null, query);
    }
