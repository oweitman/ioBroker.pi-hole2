...
class piholeserverclass {
     ...
     *
     * @param {string} clientName - Pi-hole client hostname.
     ...
     */
    async getClientQueriesForDay(clientName, from, until) {
        const queries = [];
        ...
        while (hasMore) {
            ...
            if (!result.ok || !Array.isArray(responseBody?.queries)) {
                this.adapter.log.warn(
                    `Could not get daily queries for client ${clientName}: ${result.error?.message ?? result.error ?? 'invalid response'}`,
                );
                return null;
            }
            ...
            hasMore = responseBody.queries.length >= pageSize && nextCursor !== undefined && nextCursor !== cursor;
            cursor = nextCursor;
        }
     ...
     */
    async updateClientDomainStates(clientName, objectName, queries) {
    ...
    async getClientDomainStats() {
        ...
        if (!suggestions.ok || !Array.isArray(clientNames)) {
            this.adapter.log.warn(
        ...
        const clientQueryDelay = this.calculateClientQueryDelay(uniqueClientNames.length);

        for (let index = 0; index < uniqueClientNames.length; index++) {
            const clientName = uniqueClientNames[index];
            let objectName = this.sanitizeClientName(clientName);
            ...
            const queries = await this.getClientQueriesForDay(clientName, from, until);
            if (queries) {
                await this.updateClientDomainStates(clientName, objectName, queries);
            }

            if (index < uniqueClientNames.length - 1 && clientQueryDelay > 0) {
