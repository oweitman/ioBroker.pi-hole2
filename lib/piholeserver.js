...
class piholeserverclass {
    ...
    async checkDatapoints() {
        ...
        for (const key in this.stateTemplate) {
            ...
            if (!stateTemplate?.exist) {
                ...
                if (this.data) this.data[stateTemplate.name] = stateTemplate.def;
                //                }
    ...
    async stateChange(id, state) {
        ...
        if (idParts[0] == 'BlockingTime') {
            if (this.data) this.data['BlockingTime'] = parseInt(state.val);
        }
    }
    async doToggleBlocking() {
        ...
        this.pihole && await this.pihole.setBlocking(dataBlocking.body?.blocking == 'enabled' ? false : true, blockingTime);
    ...
    async getData() {
        if (this.pihole) {
            ...
            if (this.data) this.data['Summary'] = JSON.stringify(dataSummary.body);
            ...
            if (this.data) this.data['TopClients'] = JSON.stringify(dataTopClients.body);
            ...
            if (this.data) this.data['TopDomains'] = JSON.stringify(dataTopDomains.body);
            ...
            if (this.data) this.data['History'] = JSON.stringify(dataHistory.body);
            ...
            if (this.data) this.data['Version'] = JSON.stringify(dataVersion.body);

            const dataBlocking = await this.pihole.getBlocking();
            await this.ioUtil.setStateAsync('Blocking', dataBlocking.body?.blocking == 'enabled' ? true : false, null, null);
            if (this.data) this.data['Blocking'] = dataBlocking.body?.blocking == 'enabled' ? true : false;
